...
then
        count="0"
        while ( [ "`${datastore_tool} s3://${configbucket}/$1 $2 /tmp 2>&1 >/dev/null | /bin/grep "ERROR"`" != "" ] && [ "${count}" -lt "5" ] )
        do
                /bin/sleep 5
                count="`/usr/bin/expr ${count} + 1`"
        done 
...
then
        count="0"
        while ( [ "`${datastore_tool_1} s3://${configbucket}/$1 $2 /tmp 2>&1 >/dev/null | /bin/grep "ERROR"`" != "" ] && [ "${count}" -lt "5" ] )
        do
                /bin/sleep 5
                count="`/usr/bin/expr ${count} + 1`"
        done  
